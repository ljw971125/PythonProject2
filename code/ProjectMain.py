...
class Menu1(tk.Frame):
    ...
    def __init__(self,master):
        ...
        for i in range(0,25):
            mylist.insert(tk.END, df1.loc[i][0])
        ...
        # bt3=Button(frame3,text="그래프 보기",command=graph,width=40,height=3,background='white',font=20)
        # bt3.pack(side=LEFT,expand=True,fill=BOTH)
        ...
        def show_info(event):
            ...
            index = mylist.curselection()[0]
            info = mylist.get(index)
            label.config(text=f"{info} 사고유형 분석",font=40)
...
class MainMenu(tk.Frame):
    def __init__(self, master):
        ...
        frame1 = tk.Frame()
        frame1.pack(fill=BOTH)
        bt=Button(frame1,text="사고 유형 분석",width=40,height=3,background='white',font=20,command=lambda: master.switch_frame(Menu1))
        bt.pack(side=LEFT,expand=True,fill=BOTH)
        bt2=Button(frame1,text="사고 유형 상세 분석",width=40,height=3,background='white',font=20,command=lambda: master.switch_frame(Menu2))
        bt2.pack(side=LEFT,expand=True,fill=BOTH)
        bt3=Button(frame1,text="유형별 최다 사고",width=40,height=3,background='white',font=20,command=lambda: master.switch_frame(Menu3))
        bt3.pack(side=LEFT,expand=True,fill=BOTH)
...
class Menu2(tk.Frame):
    def __init__(self,master):
        ...
        R1 = Radiobutton(frame4, text='음주운전',variable=var, value="음주운전",command=selection,font=20) # 오른쪽 프레임 체크박스 추가
        R1.pack(anchor='w') # 왼쪽으로 정렬
        R2 = Radiobutton(frame4, text='무면허',variable=var, value="무면허",command=selection,font=20)
        R2.pack(anchor='w')
        R3 = Radiobutton(frame4, text='스쿨존',variable=var, value="스쿨존",command=selection,font=20)
        R3.pack(anchor='w')
        R4 = Radiobutton(frame4, text='과속',variable=var, value="과속",command=selection,font=20)
        R4.pack(anchor='w')
        R5 = Radiobutton(frame4, text='신호위반',variable=var, value="신호위반",command=selection,font=20)
        R5.pack(anchor='w')
...
class Menu3(tk.Frame):
    def __init__(self,master):
        ...
        bt3.pack(side=LEFT,expand=True,fill=BOTH)
        bt3=Button(frame3,text="그래프 보기",width=40,height=3,background='white',font=20)
        bt3.pack(side=LEFT,expand=True,fill=BOTH)
